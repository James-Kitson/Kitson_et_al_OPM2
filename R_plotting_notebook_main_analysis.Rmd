...
read.stats<-read.csv(file="data/Richard_combined_read_stats_all.csv", stringsAsFactors=FALSE, header=TRUE)
...
read.stats$type<-ifelse(read.stats$template=="PCRneg","Negative",
                          ifelse(read.stats$template=="eneg","Negative",
                            ifelse(read.stats$template=="DNAp","DNApositive",
                              ifelse(read.stats$template=="PCRp","PCRpositive","Sample"))))
...
svg(file="diagrams/FigureS6_run2.svg",12,8)
grid.draw(g.reads)
dev.off()
...
### Write the grobbed ggplots to an svg
svg(file="diagrams/FigureS7_run2.svg",12,8)
...
###Mistagging analysis
Now that we have checked for a stable cluster size and contamination in our negatives, we should also check for mistagging in our sequencing data. To do this we have deliberately excluded certain tag combinations during PCR set up. We can now perform an analysis similar to the ones above but instead of using our tag combinations associated with samples, we use a query map with all the illegal tag combinations (84 combinations).

```{r}
### Read in the data and look at the number of clusters and read depth against minimum cluster size
null.read.stats<-read.csv(file="data/combined_illegal_read_stats_run2.csv", stringsAsFactors=FALSE, header=TRUE)

### Create a plot of read depth in PCR wells by minimum cluster size
null.reads<-ggplot(data = null.read.stats, aes(x=as.factor(clusters_min_cov), y=queries)) +
  geom_boxplot() +
  labs(y = "Per well read depth (all PCR wells)", x="Minimum cluster coverage") +
### make the labels more visible
  theme(axis.text = element_text(size = rel(1.2)),
...
### Write the grobbed ggplots to an svg
svg(file="diagrams/FigureS8_run2.svg",12,8)
null.reads
dev.off()
...
###Conclusions
Looking at the plots for per well read  and cluster depths for all samples suggests that a minimum cluster size between 46 and 61 reads results in the removal of rare sequences. Similarly we can see from the clusters and retained reads in the negatives, it is clear that removing any clusters smaller than 41 reads effectively excludes low level background contamination. Finally the mistagging analysis suggestes that aminimum read depth of >66 reads is sufficient to remove background contamination from mistagging. These values suggest that a minimum cluster size of about 67 reads will be sufficient to remove background noise in our dataset.

#Part 2: Main data analysis
In part one we decided on an a set of minimum cluster sizes for our final analysis based on three possible selection criteria. We then used these values in the [Jupyter notebook](https://github.com/HullUni-bioinformatics/Kitson_et_al_NMB/blob/master/Jupyter_notebook/OPM_NMB_main_analysis.ipynb) to run a final analysis for each minimum cluster size. Now we need to examine this output, check that negatives are clean and examine the contents of each well.

###Calculating percentage parasitism for each coverage threshold
```{r}
### make a list of output files
assignment.files <- paste("data/", grep("metaBEAT_transpose_.._run2", list.files(path="data/"), value = TRUE),sep="")
### read in the tables into a list we can loop across
assignment.list <- lapply(assignment.files, read.table, stringsAsFactors=FALSE, header=TRUE, sep="\t")
names(assignment.list)<-c("37 reads","52 reads","67 reads")

### make lists of % parasitism for each parasitoid
carcelia.list<-lapply(assignment.list, with, (colSums(assignment.list[[i]]["Carcelia_iliaca"] > 0)/1012)*100)
compsilura.list<-lapply(assignment.list, with, (colSums(assignment.list[[i]]["Compsilura_concinnata"] > 0)/1012)*100)

### make the lists into vectors and bind them into a data frame for exporting
carcelia <- do.call("rbind",carcelia.list)
compsilura <- do.call("rbind",compsilura.list)
parasitism.rates <- round(as.data.frame(cbind(carcelia,compsilura)),1)

### write this dataframe to a table for the manuscript
write.csv(parasitism.rates, file="data/Table1.csv")
```

###Calculating percentage sequencing success each coverage threshold
```{r}
### read in the combined read stats
combined.read.stats<-read.csv(file="data/combined_final_read_stats_run2.csv", stringsAsFactors = FALSE, header = TRUE)

### list the % sequencing sucess for each clustering threshold
for(i in c(37,52,67)){
print(paste("Sequencing success (all PCR wells) for minimum cluster coverage of ", i, " reads = ", 
      round((sum(subset(combined.read.stats, clusters_min_cov==i, select=queries)>0)/nrow(subset(combined.read.stats, clusters_min_cov==i)))*100,1),
      "%",sep=""))
}
```
...
```{r}
### Subset the combined read stats to the cluster coverage we will finally use
final.read.stats<-subset(combined.read.stats, clusters_min_cov == "67")

### read in the sample metadata
my.plates<-read.table(file="data/sample_metadata_run2.tsv", stringsAsFactors=FALSE, header=TRUE, sep="\t")
...
my.plates<-my.plates[, c(1:5,7)]
### name the columns
colnames(my.plates)<-c("sample", "plate", "plate.numeric", "well", "strength", "template")
...
all.samples<-ggplot(aes(y = value, x = plate, fill = variable), data = final.read.stats.subs.melt) +
  ### plot the points
  geom_point(pch = 21, position = position_jitterdodge(), stroke = 0.2) +
  ### make the boxplot and suppress the outliers as we are plotting the points anyway
  geom_boxplot(aes(fill=variable), alpha=0.5, position = position_dodge(width = 0.85), outlier.shape = NA, lwd=0.2) +
  ### fix the axes titles
...
### Plot above as a svg
svg(file="diagrams/trimming_summary_all_samples_run2.svg",12,8)
all.samples
dev.off()
...
samples.only<-ggplot(aes(y = value, x = plate, fill = variable), data = subset(final.read.stats.subs.melt, final.read.stats.subs.melt$type=="Moth sample")) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,65000)) +
  ### rotate the x-axis labels and resize the text for the svg
  theme(axis.text.x = element_text(size=rel(1.2), colour="black", angle=45, hjust=1), 
        axis.text.y = element_text(size=rel(1.2), colour="black", angle=45, hjust=1), 
        axis.title.y = element_text(size=rel(1.6), vjust=2), 
        axis.title.x = element_text(size=rel(1.6), vjust=-1.3), 
        legend.text = element_text(size = rel(1.4)), 
        legend.title = element_text(size = rel(1.3), vjust=1), 
        legend.position = "right", 
        legend.key.height=unit(2, "line"), 
        legend.key=element_blank(), 
        legend.background = element_blank(), 
...
### Plot above as a svg
svg(file="diagrams/Figure3_run2.svg",10,5)
samples.only
dev.off()
...
### Plot above as a svg
svg(file="diagrams/Figure4_run2.svg", 10, 5)
sample.types
dev.off()
...
paste("PCR success (moth samples only) = ", round((1-(sum(my.plates$strength=="fail")/nrow(counting)))*100,1),"%",sep="")
paste("Sequencing success (moth samples only) = ", round((sum(counting$queries>0)/nrow(counting))*100,1),"%",sep="")
...
paste("Standard deviation of retained cluster count for moth samples =",
      round(sd(counting$cluster_above_thres),1))
...
```{r}
my.assignments<-read.table(file="data/metaBEAT_transpose_run2.tsv", stringsAsFactors=FALSE, header=TRUE, sep="\t")
...
my.assignments$Fungi<-rowSums(my.assignments[c("Beauveria_bassiana","Penicillium","Ascomycota")])
my.assignments<-my.assignments[c(1,3:6,8,11,10)]
...
### Total all the reads
my.assignments$total<-rowSums(my.assignments[c(2:8)])
...
hit.hist<-data.frame(OTUs = rowSums(my.assignments.samps.only[c(2:8)] != 0), type=my.assignments.samps.only$type)
```
...
svg(file="diagrams/Figure8_run2.svg", width=12, height=8)
...
percent.carcelia<-(colSums(my.assignments["Carcelia_iliaca"] > 0)/1012)*100
percent.compsilura<-(colSums(my.assignments["Compsilura_concinnata"] > 0)/1012)*100
